...
int[] DURATIONS = {
  115,      // T1: 1 min 55 s (115s)
  288,      // T2: 4 min 48 s (288s)
  14400,    // T3: 4 h (14400s)
  28800     // T4: 8 h (28800s)
};
...
Button[] resetButtons = new Button[NUM_SAMPLES];
...
void setup() {
  ...
  for (int i = 0; i < NUM_SAMPLES; i++) {
   ...
   startButtons[i] = new Button(bx, by, 60, 30, "Start");
	float rx = bx + 70;
	resetButtons[i] = new Button(rx, by, 60, 30, "Reset");
...
void mousePressed() {
  for (int i = 0; i < NUM_SAMPLES; i++) {
    if (startButtons[i].isClicked(mouseX, mouseY)) {
		if (!timers[i].isAnyRunning()) {
	      timers[i].start();
		}
    }
	if (resetButtons[i].isClicked(mouseX, mouseY)) {
  timers[i].reset();
}
